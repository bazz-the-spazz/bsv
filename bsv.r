...
write.bsv <- function(x, file, description, metadata="no.metadata", rownames=FALSE, prompt.metadata=FALSE, prompt.description=FALSE,  fileEncoding = "UTF-8" ){
  ...
  d <- c(paste("fileEncoding", fileEncoding), description,  "Dimension of table (rows and columns):", dim(x),  "METADATA_for_variables",  paste(names(x), Is,  metadata, sep="__METADATA__"),
